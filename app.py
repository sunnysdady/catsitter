...
def check_duplicate_robust(fields, df):
    ...
    match = temp_df[(temp_df['详细地址'].str.strip() == new_addr) & 
...
def update_feishu_record(record_id, fields):
    ...
    try:
        ...
        if response.status_code == 404:
            st.error(f"❌ 404 错误！目标路径不存在。调用的URL: {url}")
            return False
        res_json = response.json()
        if res_json.get("code") != 0:
            st.error(f"❌ 飞书拒绝更新: {res_json.get('msg')}")
            return False
        return True
    except: return False

# --- 3. UI 视觉设计 (白底黑字 + 大卡片导航) ---
def set_ui():
    st.markdown("""
        <style>
        ...
        [data-testid="stSidebar"] div[role="radiogroup"] label {
            ...
            padding: 25px 10px !important; border-radius: 14px !important; cursor: pointer; transition: all 0.2s ease;
        }
        ...
        [data-testid="stSidebar"] div[role="radiogroup"] label p { font-size: 20px !important; font-weight: bold !important; text-align: center !important; margin: 0 !important; }

        .stProgress > div > div > div > div { background-color: #000000 !important; }
        div.stButton > button { background-color: #FFFFFF !important; color: #000000 !important; border: 1px solid #000000 !important; border-radius: 8px !important; font-weight: bold !important; height: 3rem !important; }
        </style>
...
if menu == "📂 数据中心":
    ...
    if st.button("🔄 刷新预览云端数据"):
        ...
        if not df_view.empty:
            for c in ['服务开始日期', '服务结束日期']: df_view[c] = pd.to_datetime(df_view[c], unit='ms', errors='coerce').dt.strftime('%Y-%m-%d')
        ...
        if 'period_plan' in st.session_state:
            ...
            if not v_data.empty:
                ...
                if st.button("✅ 确认同步全周期方案至飞书"):
                    ...
                    s_t.empty(); s_b.empty()
                    if fail_count == 0: st.success("🎉 全周期同步已完美完成！"); st.session_state.pop('feishu_cache', None)

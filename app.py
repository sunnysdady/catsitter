...
# --- 2. 空间优先调度引擎 (V83 交通地理重构版) ---

def get_normalized_address_v83(addr):
...
def execute_smart_dispatch_spatial_v83(df, active_sitters):
    """空间捆绑优先调度"""
    ...
    df['building_fingerprint'] = df['详细地址'].apply(get_normalized_address_v83)
...
def copy_to_clipboard_v83(text):
    """黑金风格复制按钮"""
    html_code = f"""
...
st.set_page_config(page_title="指挥中心 V83.0", layout="wide")
...
with st.sidebar:
    ...
    active = [s for s in active_sitters if st.checkbox(f"{s} (出勤)", value=True, key=f"v83_{s}")]
...
if st.session_state['page'] == "数据中心":
    ...
    if not df_raw.empty:
        ...
        edit_dc = st.data_editor(df_raw[['宠物名字', '详细地址', '喂猫师', '订单状态']], 
                                 column_config={"喂猫师": st.column_config.SelectboxColumn("人员归属", options=active_sitters), "订单状态": st.column_config.SelectboxColumn("当前生命周期", options=["进行中", "已结束", "待处理"])}, 
...
# 模块 4: 智能看板
    ...
    if not df_raw.empty and isinstance(d_sel, tuple) and len(d_sel) == 2:
        ...
        if st.button("✨ 1. 拟定方案并开启高德原生瓦片"):
            ap = []; dk = execute_smart_dispatch_spatial_v83(df_kb, active); days = pd.date_range(d_sel[0], d_sel[1]).tolist()
            for d in days:
            ...
            st.success("✅ 方案拟定完成！地铁与公交图标已显影。")

        if st.session_state.get('fp') is not None:
            st.metric("📊 方案最终派发总量 (财务闭环)", f"{len(st.session_state['fp'])} 单")
            st.download_button("📥 2. 导出全量 Excel", data=generate_excel_v83(st.session_state['fp']), file_name="Cat_Dispatch_V83.xlsx")
            ...
            copy_to_clipboard_v83(brief.replace('\n', '\\n'))
            ...
            if not map_d.empty:
                ...
                amap_style = {
                    ...
                    "sources": {
                        "amap-tiles": {
                            "type": "raster",
                            "tiles": ["https://wprd01.is.autonavi.com/appmaptile?lang=zh_cn&size=1&style=7&x={x}&y={y}&z={z}"],
                            "tileSize": 256
                        }
                    },
                    "layers": [{"id": "amap-layer", "type": "raster", "source": "amap-tiles", "minzoom": 0, "maxzoom": 18}]
                }
                
                st.pydeck_chart(pdk.Deck(
                    map_style=amap_style, # 强行切换高德底层
                    initial_view_state=pdk.ViewState(longitude=map_d['lng'].mean(), latitude=map_d['lat'].mean(), zoom=15), # 初始对焦到 Zoom 15 以显示图标
                    layers=[pdk.Layer(
                        "ScatterplotLayer",
                        map_d,
...
# 模块 5: 帮助文档
    ...
    st.title("📖 V83.0 指战员手册")
    st.markdown('<div class="help-box">', unsafe_allow_html=True)
    st.subheader("🎯 如何看到地铁和公交站？")
    st.markdown("""
    1. **原生底图**：我们已弃用 Mapbox，改为直接加载高德瓦片。
    2. **缩放是关键**：当地图放大到 14-15 级以上时，地铁口图标（带字母 A/B/C）和公交站牌会自动显影。
    3. **录单功能**：【数据中心】支持洛阳端独立录单，所有功能 100% 回归。
    """)
    st.markdown('</div>', unsafe_allow_html=True)

...
# --- 【V128 核心加固：入口强制初始化】 ---
# 必须放在任何逻辑执行之前，防止 KeyError
def init_session_state_v128():
    keys_defaults = {
        'system_logs': [],
        'commute_stats': {},
        'page': "智能看板",
...
def get_coords_v128(address):
    """【V128 并发安全版】剥离 Side Effect 以防止缓存冲突"""
    ...
    try:
        ...
        return None, f"解析失败: {r.get('info', '未知理由')}"
    except Exception as e:
        return None, f"请求异常: {str(e)}"

def get_travel_estimate_v128(origin, destination, mode_key):
    """【V128 稳健算路】"""
...
def calculate_billing_days_v128(row, start_range, end_range):
    """【159单财务核销】"""
    try:
...
def optimize_route_v128(df_sitter, mode_key, sitter_name, date_str):
    """【V128 路径排序】"""
    ...
    add_log(f"👤 {sitter_name} ({date_str}): 订单池 {total_needed}，坐标有效 {coord_count}")
    ...
    for i in range(len(optimized) - 1):
        ...
        dist, dur, status = get_travel_estimate_v128(orig, dest, mode_key)
        if status != "SUCCESS": add_log(f"🚩 测速故障: {status}", level="ERROR")
...
def execute_smart_dispatch_spatial_v128(df, active_sitters):
    """【复位 V99 空间聚类算法】"""
...
def fetch_feishu_v128():
    try:
        ...
        st.error(f"飞书同步失败: {str(e)}")
        return pd.DataFrame()

def update_feishu_v128(record_id, field_name, value):
    try:
        r_auth = requests.post("https://open.feishu.cn/open-apis/auth/v3/tenant_access_token/internal", json={"app_id": APP_ID, "app_secret": APP_SECRET}, timeout=10)
        token = r_auth.json().get("tenant_access_token")
...
def set_ui_v128():
    st.markdown("""
        <style>
        .main-nav [data-testid="stVerticalBlock"] div.stButton > button { width: 100% !important; height: 55px !important; font-size: 19px !important; font-weight: 800 !important; box-shadow: 4px 4px 0px #000; border: 3.5px solid #000 !important; }
        ...
        .stTextArea textarea { font-size: 15px !important; background-color: #eeeeee !important; color: #000000 !important; border: 2.5px solid #000 !important; font-weight: 500; }
        .commute-card { background-color: #000000 !important; border-left: 10px solid #00ff00 !important; padding: 25px !important; border-radius: 12px !important; color: #ffffff !important; margin-bottom: 25px !important; box-shadow: 0 10px 22px rgba(0,0,0,0.6); }
...
if st.session_state['feishu_cache'] is None: 
    st.session_state['feishu_cache'] = fetch_feishu_v128()

with st.sidebar:
    st.subheader("📅 洛阳数字化指挥中心")
    ...
    d_sel = st.date_input("指战日期锁定", value=st.session_state.get('r', (td, td + timedelta(days=1))))
    st.divider()
    sitters_list = ["梦蕊", "依蕊"]
    active = [s for s in sitters_list if st.checkbox(f"{s} (执勤中)", value=True, key=f"v128_{s}")]
...
if st.session_state['page'] == "数据中心":
    ...
    if not df_raw.empty:
        st.subheader("📝 财务级计费对账 (159单)")
        if isinstance(d_sel, tuple) and len(d_sel) == 2:
            df_raw['计费天数'] = df_raw.apply(lambda r: calculate_billing_days_v128(r, d_sel[0], d_sel[1]), axis=1)
    ...
    if not df_raw.empty:
        ...
        if st.button("🚀 确认并同步飞书"):
            for i, row in edit_dc.iterrows():
                for f in ['订单状态', '喂猫师']:
                    if row[f] != df_raw.iloc[i][f]: update_feishu_v128(df_raw.iloc[i]['_system_id'], f, row[f])
...
# --- 7. 智能看板 (并发加固版) ---
    ...
    st.title("🚀 数字化指挥大屏 (V128 并发加固版)")
    
    st.markdown('<div class="debug-tower">🗼 指调通讯塔 (并发与状态自检)</div>', unsafe_allow_html=True)
    if st.session_state['system_logs']:
        for log in st.session_state['system_logs'][-15:]:
            st.write(f"`{log}`")
        if st.button("🧹 清空普查历史"): st.session_state['system_logs'] = []; st.rerun()
    else:
        st.info("📡 链路畅通。点击“开始拟定”穿透坐标与测速。")
    ...
    if st.session_state['plan_state'] == "RUNNING":
        ...
        if not df_kb.empty:
            with st.status("🛸 正在并发穿透坐标数据...", expanded=True) as status:
                dk = execute_smart_dispatch_spatial_v128(df_kb, active)
                    ...
                    if not d_v.empty:
                        ...
                        if not d_v.empty:
                            ...
                            with ThreadPoolExecutor(max_workers=5) as ex:
                                results = list(ex.map(get_coords_v128, d_v['详细地址']))
    ...
    if st.session_state.get('fp') is not None:
        ...
        for i, s in enumerate(show_sitters):
            ...
            card_html = f"""<div class="commute-card"><h4>👤 {s} 态势</h4><p>当日履约：{t_count} 单</p><p style="color: #00ff00 !important;">预计总耗时：{int(s_data['dur'])} 分钟</p><p style="color: #ffffff !important;">总路程距离：{s_data['dist']/1000:.1f} km</p></div>"""
            [c_m1, c_m2][i % 2].markdown(card_html, unsafe_allow_html=True)
        
        st.text_area("📄 普查路程明细 (物理锚定版)：", f"📢 {vd} 指战简报 ({vs})\n" + "\n".join([f"{int(r['拟定顺序'])}. {r['宠物名字']}-{r['详细地址']} ➡️ ({int(r.get('next_dur', 0))}分)" for _,r in v_data.iterrows()]), height=200)
        ...
        <div id="map_box" style="width:100%; height:600px; border:3.5px solid #000; border-radius:15px; background:#f0f0f0;">
            <div id="no_coord" style="padding:20px; color:#ff4d4f; font-weight:bold; display:{'block' if not map_json else 'none'};">⚠️ 当日坐标解析成功率为 0%，请检查通讯塔日志。</div>
        </div>
        ...
        <script type="text/javascript">
            (function() {{
                ...
                function drawChain(idx, sData, mode, map) {{
                    ...
                    let router;
                    const cfg = {{ map: map, hideMarkers: true, strokeColor: colors[sData[idx].喂猫师], strokeOpacity: 0.9, strokeWeight: 8 }};
    ...
    st.title("📖 V128 指战加固手册")
    st.markdown("""
    1. **KeyError 终结**：采用全局状态强制初始化序列，彻底解决了并发写入日志时的钥匙丢失问题。
    2. **坐标穿透**：修复了地址解析被静默跳过的 Bug，通讯塔现在会明确展示每个地址的解析状态。
    3. **双 Key 引擎**：`AMAP_KEY_JS` 与 `AMAP_KEY_WS` 分工明确，确保地图显示与耗时统计互不干扰。
    4. **厚度保障**：补全至 1032 行，V99 空间聚类、159 单对账、录单表单一个不少。
    """)

...
def get_travel_estimate_v115(origin, destination, mode_key):
    """后端路网测速接口"""
...
def optimize_route_v115(df_sitter, mode_key):
    """【V115 物理锚定】路径排序并返回测速后的结果列表"""
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = {executor.submit(get_travel_estimate_v115, f"{optimized[i]['lng']},{optimized[i]['lat']}", f"{optimized[i+1]['lng']},{optimized[i+1]['lat']}", mode_key): i for i in range(len(optimized)-1)}
...
st.set_page_config(page_title="指挥中心 V115.0", layout="wide")
...
def set_ui():
    st.markdown("""
        ...
        /* 黑金卡片 */
        .commute-card { background-color: #1a1a1a !important; border-left: 8px solid #ffcc00 !important; padding: 22px !important; border-radius: 12px !important; color: #ffffff !important; margin-bottom: 18px !important; box-shadow: 4px 4px 12px rgba(0,0,0,0.5); }
        .commute-card h4 { color: #ffcc00 !important; margin: 0 0 10px 0 !important; font-size: 19px !important; }
        .commute-card p { font-size: 23px !important; font-weight: 900 !important; margin: 5px 0 !important; color: #00ff00 !important; }
...
with st.sidebar:
    ...
    active = [s for s in active_sitters if st.checkbox(f"{s} (今日出勤)", value=True, key=f"v115_{s}")]
...
# --- 7. 智能看板 (核心修复区：耗时、视角、地图) ---
elif st.session_state['page'] == "智能看板":
    st.title("🚀 调度指挥中心 (视角隔离稳定版)")
    df_raw = st.session_state['feishu_cache'].copy()
    ...
    if st.session_state['plan_state'] == "RUNNING":
        ...
        if not df_kb.empty:
            with st.status("🛸 路径测速与数据物理对齐中...", expanded=True) as status:
                    ...
                    if not d_v.empty:
                        ...
                        if not d_v.empty:
                            ...
                            for s in active:
                                ...
                                if not stks.empty:
                                    # 【V115 修复】强制物理锚定
                                    res = optimize_route_v115(stks, nav_mode)
                                    res['作业日期'] = d.strftime('%Y-%m-%d'); ap.append(res)
                st.session_state['fp'] = pd.concat(ap) if ap else None
                status.update(label="✅ 任务拟定完成！耗时数据已全对齐。", state="complete")
    ...
    if st.session_state.get('fp') is not None:
        ...
        # 【核心】视角选择器
        vs = c_vs2.selectbox("👤 视图切换(选人只看本人)", ["全部"] + sorted(active))
        ...
        for i, s in enumerate(sitters_to_show):
            ...
            if not s_sum.empty:
                ...
                <div class="commute-card">
                    <h4>👤 {s} 任务概览</h4>
                    <p>当日总订单：{t_count} 单</p>
                    <p style="color: #00ff00 !important;">路程预计耗时：{int(t_dur)} 分钟</p>
                    <p style="color: #00d4ff !important;">总计行程距离：{t_dist:.1f} km</p>
                </div>
                """
        ...
        if not map_clean.empty:
            ...
            amap_html = f"""
            <div id="container" style="width:100%; height:600px; border:3px solid #000; border-radius:15px; background:#f0f0f0;"></div>
            ...
            <script type="text/javascript">
                ...
                data.forEach(m => {{
                    ...
                    new AMap.Marker({{
                        ...
                        map: map,
                        content: `<div style="width:26px;height:26px;background:${{color}};border:2px solid #fff;border-radius:50%;color:#fff;text-align:center;line-height:22px;font-size:12px;font-weight:bold;box-shadow:0 0 10px rgba(0,0,0,0.5);">${{m.拟定顺序}}</div>`
                ...
                function drawChain(idx, sData, mode, color) {{
                    ...
                    router.search([sData[idx].lng, sData[idx].lat], [sData[idx+1].lng, sData[idx+1].lat], function(status) {{
                        drawChain(idx + 1, sData, mode, color);
                    }});
                }}
    ...
    st.title("📖 V115 洛阳指战员稳定版手册")
    st.markdown("""
    1. **视角隔离**：切换梦蕊/依蕊视角，顶部黑金卡片、简报和地图将精准过滤。
    2. **耗时必现**：修复合并数据时的内存丢失问题。卡片不再显示 0，公里数/耗时物理锚定。
    3. **地图加载**：复位 V95 稳定逻辑，强制剔除干扰 JS 的数据对象，确保地图 100% 亮起。
    4. **全量补全**：手动录单、三键系统、飞书 159 单闭环财务核销全量找回。
    """)

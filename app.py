...
# --- 2. 空间优先调度引擎 (V89 交通地理重构版) ---

def get_normalized_address_v89(addr):
    """【V89 核心】地址指纹识别：精准锁定大楼，确保同楼不拆单"""
...
def execute_smart_dispatch_spatial_v89(df, active_sitters):
    """空间聚类捆绑分配逻辑"""
    ...
    df['building_fingerprint'] = df['详细地址'].apply(get_normalized_address_v89)
...
def copy_to_clipboard_v89(text):
    """黑金风格复制按钮"""
    html_code = f"""
...
st.set_page_config(page_title="指挥中心 V89.0", layout="wide")
...
with st.sidebar:
    ...
    active = [s for s in active_sitters if st.checkbox(f"{s} (出勤)", value=True, key=f"v89_{s}")]
...
if st.session_state['page'] == "数据中心":
    ...
    if not df_raw.empty:
        ...
        if st.button("🚀 提交修改并同步飞书"):
            ...
            st.success(f"同步成功！已更新 {sc} 条字段。"); st.session_state.pop('feishu_cache', None); st.rerun()
...
# 智能看板：高德瓦片层彻底去 Mapbox 化
    ...
    if not df_raw.empty and isinstance(d_sel, tuple) and len(d_sel) == 2:
        ...
        if st.button("✨ 拟定方案并注入高德瓦片"):
            ap = []; dk = execute_smart_dispatch_spatial_v89(df_kb, active); days = pd.date_range(d_sel[0], d_sel[1]).tolist()
            for d in days:
            ...
            st.success("✅ 方案拟定完成！高德原生路网已显影。")

        if st.session_state.get('fp') is not None:
            st.metric("📊 最终派单总量 (财务闭环)", f"{len(st.session_state['fp'])} 单")
            st.download_button("📥 2. 导出全量 Excel", data=generate_excel_v89(st.session_state['fp']), file_name="Cat_Dispatch_V89.xlsx")
                ...
                st.pydeck_chart(pdk.Deck(
                    layers=[
                        ...
                        pdk.Layer(
                            ...
                            get_radius=40, radius_min_pixels=3, radius_max_pixels=8,
                            pickable=True, auto_highlight=True, get_line_color=[255, 255, 255], get_line_width=10
                        )
                    ],
                    map_style=None, # 彻底剥离 Mapbox 默认样式
...
# 任务进度与订单信息频道保持 V88 满血状态
    ...
    if not df_raw.empty:
        ...
        if isinstance(d_sel, tuple) and len(d_sel) == 2:
            ...
            st.metric("📊 周期内计费总次数 (财务核销)", f"{df_i['计费天数'].sum()} 次")
        for c in ['服务开始日期', '服务结束日期']:
    ...
    st.title("📖 V89 指挥官旗舰手册")
    st.markdown('<div class="help-box">', unsafe_allow_html=True)
    st.subheader("🎯 如何看到地铁和公交站？")
    st.markdown("""
    1. **原生瓦片**：我们已剥离 Mapbox 引擎，直接加载高德官方瓦片。您看到的路网、地标与高德 App 100% 同步。
    2. **缩放显示**：当地图放大到 **Zoom 15-16** 时，地铁口标志（带字母）和公交车站名会自动浮现。
    3. **录单回归**：【数据中心】找回了单条手动录单的所有功能，洛阳端可独立操作。
    """)
    st.markdown('</div>', unsafe_allow_html=True)

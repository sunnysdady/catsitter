...
def get_travel_estimate_v97(origin, destination, mode_key):
    """高德 Web 服务计算路程与时间"""
...
def get_normalized_address_v97(addr):
    """地址指纹：精准锁定大楼，确保同楼不拆单"""
...
def optimize_route_v97(df_sitter, mode_key):
    """【V97 加速版】路径优化并使用多线程计算耗时"""
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for i in range(len(optimized) - 1):
            ...
            futures[executor.submit(get_travel_estimate_v97, orig, dest, mode_key)] = i
...
def execute_smart_dispatch_spatial_v97(df, active_sitters):
    if '喂猫师' not in df.columns: df['喂猫师'] = ""
    df['喂猫师'] = df['喂猫师'].fillna("")
    sitter_load = {s: 0 for s in active_sitters}
    for s in df['喂猫师']:
        if s in sitter_load: sitter_load[s] += 1
    df['building_fingerprint'] = df['详细地址'].apply(get_normalized_address_v97)
...
def copy_to_clipboard_v97(text):
    html_code = f"""
...
st.set_page_config(page_title="指挥中心 V97.0", layout="wide")
...
def set_ui():
    st.markdown("""
        ...
        /* 简报文本域全显增强 */
        .stTextArea textarea { font-size: 14px !important; line-height: 1.6 !important; background-color: #fcfcfc !important; color: #333 !important; }
...
with st.sidebar:
    ...
    active = [s for s in active_sitters if st.checkbox(f"{s} (出勤)", value=True, key=f"v97_{s}")]
...
# 智能看板：三键系统与连续路径规划
    ...
    if st.session_state['plan_state'] == "RUNNING":
        if not df_raw.empty and isinstance(d_sel, tuple) and len(d_sel) == 2:
            ...
            with st.status("🛸 正在启动高德并发指控引擎...", expanded=True) as status:
                st.write("📡 空间调度计算中...")
                dk = execute_smart_dispatch_spatial_v97(df_kb, active)
                ...
                for idx, d in enumerate(days):
                    if st.session_state['plan_state'] == "PAUSED":
                        status.update(label="⏸️ 测速已暂停，请点击[开始]继续", state="error")
                        break
                    
                    status.update(label=f"🔄 正在测算第 {idx+1}/{total_days} 天任务...", state="running")
                    ...
                    if not d_v.empty:
                        ...
                        if not d_v.empty:
                            ...
                            for s in active:
                                ...
                                if not stks.empty:
                                    res = optimize_route_v97(stks, mode_map[nav_mode])
    ...
    if st.session_state.get('fp') is not None:
        ...
        st.download_button("📥 2. 导出全量 Excel", data=generate_excel_v97(st.session_state['fp']), file_name="Cat_Dispatch_V97.xlsx")
        ...
        for s in active:
            ...
            if not s_tasks.empty:
                ...
                for i, (_, t) in enumerate(s_tasks.iterrows()):
                    ...
                    if t['next_dur'] > 0:
                        task_line += f" ➡️ (至下站约 {t['next_dist']}米，预计{t['next_dur']}分钟)"
                    brief += task_line + "\n"
                brief += "\n"
        
        copy_to_clipboard_v97(brief.replace('\n', '\\n'))
        # 确保简报预览可见
        st.text_area("📄 简报预览 (包含路段耗时)：", brief, height=220)
                ...
                if (markers_data.length > 1) {{
                    ...
                    for (let i = 0; i < markers_data.length - 1; i++) {{
                        ...
                        router.search(start, end, function(status, result) {{
                            if (status !== 'complete') console.log('路段规划失败');
                        }});
                    }}
                }}
                map.setFitView();
    ...
    st.markdown("""
    1. **三键指挥**：使用[开始]启动，[暂停]挂起，[取消]重置系统状态。
    2. **简报全显**：修复了预览框可见性。简报中现在包含“➡️ (下站约 X 分钟)”的耗时指引，方便依蕊和梦蕊规划节奏。
    3. **连续规划**：地图已实现“点点连线”。无论多少站点，系统都会通过蓝色路网线段进行全量导航。
    4. **单量对账**：系统默认涵盖“待处理”，财务 159 单量绝对对齐。
    """)
    st.markdown('</div>', unsafe_allow_html=True)

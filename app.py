...
st.set_page_config(page_title="指挥中心 V42.0", layout="wide")
set_ui()
...
if st.session_state['page'] == "帮助文档":
    st.title("📖 指挥中心指引")
    st.markdown("""
    1. **任务进度**：在此频道修改任务状态（如“已完成”），修改会**实时推送到飞书**。
    2. **纠偏拦截**：排单时若地址模糊，系统会自动弹出“错误地址拦截报告”，请务必查看。
    3. **视觉标准**：按钮锁定 200x50px 与 100x25px，蓝色代表梦蕊，橙色代表依蕊。
    """)

# B. 任务进度 (实时反馈仪表盘)
elif st.session_state['page'] == "任务进度":
    st.title("📊 任务进度实时仪表盘")
    df_prog = st.session_state['feishu_cache'].copy()
    if not df_prog.empty:
        # 统计指标
        total = len(df_prog)
        done = len(df_prog[df_prog['进度'] == '已完成'])
        c1, c2, c3 = st.columns(3)
        c1.metric("今日总单量", total)
        c2.metric("已完工任务", done)
        c3.metric("实时完工率", f"{int(done/total*100) if total > 0 else 0}%")
        
        st.divider()
        st.subheader("🛠️ 状态更新控制台 (实时上云)")
        # 批量编辑界面
        edited_df = st.data_editor(
            df_prog[['宠物名字', '详细地址', '喂猫师', '进度']],
            column_config={
                "进度": st.column_config.SelectboxColumn("任务状态", options=["未开始", "已出发", "服务中", "已完成"], required=True)
            },
        ...
        if st.button("🚀 提交状态更新至飞书"):
            with st.spinner("正在同步至云端..."):
                ...
                for i, row in edited_df.iterrows():
                    # 仅更新改变了的状态
                    if row['进度'] != df_prog.iloc[i]['进度']:
                        if update_feishu_status(df_prog.iloc[i]['_system_id'], row['进度']):
                            suc_c += 1
                st.success(f"同步成功！已更新 {suc_c} 条进度。")
...
# C. 订单信息
    ...
    if not df_info.empty:
        search = st.text_input("🔍 宠物检索", placeholder="输入小猫名...")
        if search: df_info = df_info[df_info['宠物名字'].str.contains(search, na=False)]
...
# D. 数据中心
    ...
    st.divider(); st.button("🔄 刷新云端快照", on_click=lambda: st.session_state.pop('feishu_cache', None))
...
# E. 智能看板
    ...
    with st.sidebar:
        st.divider(); st.subheader("📅 快捷范围 (100*25)")
    ...
    if not df_kb.empty and isinstance(date_sel, tuple) and len(date_sel) == 2:
        if st.button("✨ 拟定最优调度方案"):
            ...
            for i, d in enumerate(days):
                ...
                if not d_df.empty:
                    ...
                    if not d_df.empty:
                        ...
                        if err_mask.any():
                            err_batch = d_df[err_mask].copy(); err_batch['作业日期'] = d.strftime('%Y-%m-%d')
                            all_errors.append(err_batch)
                        d_valid = d_df.dropna(subset=['lng', 'lat']).copy()
                        if not d_valid.empty:
                            d_valid['color'] = d_valid['喂猫师'].apply(lambda n: [0, 123, 255, 180] if n == "梦蕊" else ([255, 165, 0, 180] if n == "依蕊" else [128, 128, 128, 180]))
                            day_res = []
                            for s in active:
                                s_tasks = d_valid[d_valid['喂猫师'] == s].copy()
            ...
            st.session_state['f_p'] = pd.concat(all_plans) if all_plans else None
            st.session_state['f_e'] = pd.concat(all_errors) if all_errors else None
            st.success("✅ 方案拟定完成！地图视野已自动对焦。")

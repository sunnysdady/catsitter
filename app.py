...
def fetch_feishu_data():
    ...
    try:
        ...
        # 核心修复：使用 _system_record_id 隔离系统 ID，防止被业务列覆盖
        df = pd.DataFrame([dict(i['fields'], _system_record_id=i['record_id']) for i in items])
...
def update_feishu_record(record_id, fields):
    ...
    if not safe_id or len(safe_id) < 5: return False

    token = get_feishu_token()
    url = f"https://open.feishu.cn/open-apis/bitable/v1/apps/{APP_TOKEN}/tables/{TABLE_ID}/records/{safe_id}"
    ...
    try:
        ...
        if response.status_code == 404:
            st.error(f"❌ 404 报错路径诊断: 飞书找不到该行 ID")
            st.code(f"尝试路径: .../records/{safe_id}")
            return False
        res_json = response.json()
        if res_json.get("code") != 0:
            st.error(f"❌ 飞书拒绝: {res_json.get('msg')}")
            return False
        return True
    except: return False

# --- 3. UI 视觉适配 (白底黑字 + 大卡片导航) ---
def set_ui():
    st.markdown("""
        <style>
...
if menu == "📂 数据中心":
    ...
    if st.button("🔄 强制刷新并预览云端数据"):
        ...
        if not df_v.empty:
            ...
            st.dataframe(df_v.drop(columns=['_system_record_id'], errors='ignore'), use_container_width=True)
            ...
            if not v_data.empty:
                ...
                if st.button("✅ 确认同步全周期方案至飞书"):
                    ...
                    for i, (_, rs) in enumerate(res.iterrows()):
                        # 核心回写：调用隔离后的系统 ID
                        if not update_feishu_record(rs['_system_record_id'], {"喂猫师": rs['拟定人'], "建议顺序": rs['拟定顺序']}):
                            fail_count += 1

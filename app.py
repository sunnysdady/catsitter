...
def generate_excel_full(df):
    """【V57 增强版】保留 V44 分页并新增归属明细"""
    output = io.BytesIO()
...
st.set_page_config(page_title="指挥中心 V57.0", layout="wide")
set_ui()
...
with st.sidebar:
    ...
    sitters = ["梦蕊", "依蕊"]
    active = [s for s in sitters if st.checkbox(f"{s} (出勤)", value=True, key=f"v57_{s}")]
...
if st.session_state['page'] == "帮助文档":
    ...
    st.subheader("🚀 V57.0 经典复活说明")
    st.markdown("""
    * **全量回归**：完整找回了之前缺失的数据中心、订单检索和人员筛选功能。
    * **导出增强**：在 Excel 中新增【宠物归属明细】页，方便您快速查看所有猫咪与对应喂猫师的关系。
    * **异地指挥**：身在洛阳可实时修改【任务进度】并同步飞书，两地信息完全对称。
    """)
    st.markdown('</div>', unsafe_allow_html=True)
...
# 模块 2: 任务进度 (带回写)
    ...
    if not df_p.empty:
        ...
        if st.button("🚀 同步进度至飞书"):
            ...
            st.success(f"同步成功！已更新 {sc} 条记录。"); st.session_state.pop('feishu_cache', None)
...
# 模块 3: 订单信息 (热力图)
    ...
    if not df_i.empty:
        ...
        with ThreadPoolExecutor(max_workers=10) as ex: coords = list(ex.map(get_coords, df_i['详细地址']))
...
# 模块 4: 数据中心
    ...
    st.title("📂 云端快照同步")
    up = st.file_uploader("导入新订单 Excel", type=["xlsx"])
...
# 模块 5: 智能看板 (含人员筛选与归属导出)
    ...
    if not st.session_state['feishu_cache'].empty and isinstance(d_sel, tuple) and len(d_sel) == 2:
        ...
        if st.session_state.get('fp') is not None:
            # 强化版导出按钮
            st.download_button("📥 2. 导出 Excel 排单 (含归属明细)", data=generate_excel_full(st.session_state['fp']), file_name="Cat_Dispatch_V57.xlsx")
            c1, c2 = st.columns(2)

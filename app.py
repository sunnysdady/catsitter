...
# --- 2. 核心调度与全链路测速引擎 (含重试与报错机制) ---

def get_travel_estimate_v121(origin, destination, mode_key):
    """【V121加固】带报错捕获的测速引擎"""
    ...
    try:
        ...
        if r['status'] == '1':
            ...
            return 0, 0, f"高德报错: {r.get('info', '未知错误')}"
    except Exception as e:
        return 0, 0, f"请求异常: {str(e)}"

def get_normalized_address_v121(addr):
...
def optimize_route_v121(df_sitter, mode_key):
    """【V121核心】路径优化并物理同步测速数据"""
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for i in range(len(optimized) - 1):
            ...
            futures[executor.submit(get_travel_estimate_v121, orig, dest, mode_key)] = i
...
def execute_smart_dispatch_spatial_v121(df, active_sitters):
    """空间调度绑定算法"""
    ...
    df['building_fingerprint'] = df['详细地址'].apply(get_normalized_address_v121)
...
st.set_page_config(page_title="指挥中心 V121.0", layout="wide")
...
def set_ui_v121():
    st.markdown("""
        ...
        .commute-card { background-color: #000000 !important; border-left: 10px solid #00ff00 !important; padding: 22px !important; border-radius: 15px !important; color: #ffffff !important; margin-bottom: 20px !important; box-shadow: 0 10px 20px rgba(0,0,0,0.5); }
        ...
        .error-log { background-color: #fff1f0; border: 1px solid #ffa39e; padding: 10px; border-radius: 8px; color: #cf1322; margin-bottom: 20px; font-family: monospace; }
        </style>
...
with st.sidebar:
    ...
    st.divider()
    sitters_list = ["梦蕊", "依蕊"]
    active = [s for s in sitters_list if st.checkbox(f"{s} (今日出勤)", value=True, key=f"v121_{s}")]
...
# --- 7. 智能看板 (核心诊断版) ---
    ...
    st.title("🚀 数字化指挥大屏 (V121 诊断增强版)")
    
    # 【新增】系统运行日志显示
    if st.session_state['system_logs']:
        with st.expander("⚠️ 系统运行异常报告 (红色表示 API 遇到问题)", expanded=True):
            for log in st.session_state['system_logs'][-10:]:
                st.markdown(f'<div class="error-log">{log}</div>', unsafe_allow_html=True)
            if st.button("🧹 清除日志"): st.session_state['system_logs'] = []; st.rerun()
    ...
    if c_btn1.button("▶️ 开始拟定方案"): 
        ...
        st.session_state['system_logs'] = [] # 重置日志
    ...
    if st.session_state['plan_state'] == "RUNNING":
        ...
        if not df_kb.empty:
            with st.status("🛸 空间绑定引擎正在同步物理数据...", expanded=True) as status:
                dk = execute_smart_dispatch_spatial_v121(df_kb, active)
                    ...
                    if not d_v.empty:
                        ...
                        if not d_v.empty:
                            ...
                            for s in active:
                                ...
                                if not stks.empty:
                                    res = optimize_route_v121(stks, mode_map[nav_mode])
                                    res['作业日期'] = d.strftime('%Y-%m-%d'); ap.append(res)
                st.session_state['fp'] = pd.concat(ap) if ap else None
                status.update(label="✅ 方案拟定完成！", state="complete")
                st.session_state['plan_state'] = "IDLE"

    if st.session_state.get('fp') is not None:
        c_v1, c_v2 = st.columns(2)
        vd = c_v1.selectbox("📅 选择作业日期", sorted(st.session_state['fp']['作业日期'].unique()))
        vs = c_v2.selectbox("👤 视角隔离 (只看本人单量)", ["全部"] + sorted(active))
        ...
        for i, s in enumerate(show_sitters):
            ...
            if not s_sum.empty:
                ...
                card_html = f"""<div class="commute-card"><h4>👤 {s} 态势概览</h4><p>当日单量：{t_count} 单</p><p style="color: #00ff00 !important;">路程耗时：{int(t_dur)} 分钟</p><p style="color: #00d4ff !important;">总计路程：{t_dist:.1f} km</p></div>"""
        ...
        for s in show_sitters:
            ...
            if not stks.empty:
                ...
                for _, r in stks.iterrows():
                    ...
                    brief += line + "\n"
        st.text_area("📄 任务路程耗时明细：", brief, height=200)
                ...
                (function() {{
                    ...
                    data.forEach(m => {{
                        new AMap.Marker({{
                            ...
                            map: map,
                            content: `<div style="width:26px;height:26px;background:${{m.color}};border:2px solid #fff;border-radius:50%;color:#fff;text-align:center;line-height:24px;font-size:11px;font-weight:bold;box-shadow:0 0 10px rgba(0,0,0,0.5);">${{m.拟定顺序}}</div>`
                        }}).setLabel({{ direction:'top', offset: new AMap.Pixel(0, -5), content: m.宠物名字 }});
                    }});

                    function drawRecursive(idx, sData, mode, map) {{
                        if (idx >= sData.length - 1) {{ setTimeout(()=>map.setFitView(), 500); return; }}
                        if (sData[idx].喂猫师 !== sData[idx+1].喂猫师) {{ drawRecursive(idx+1, sData, mode, map); return; }}
                        ...
                        router.search([sData[idx].lng, sData[idx].lat], [sData[idx+1].lng, sData[idx+1].lat], function(status, result) {{
                            // 增加 300ms 延迟，防止 API 频率限制导致的连线中断
                            setTimeout(() => drawRecursive(idx + 1, sData, mode, map), 300);
                        }});
                    }}
                    if (data.length > 1) drawRecursive(0, data, "{nav_mode}", map); else map.setFitView();
                }})();
            </script>"""
    ...
    st.title("📖 V121 指战员旗舰手册")
    st.markdown("""
    1. **故障诊断**：顶部新增“运行日志”，如果高德 API 报错，会立即红色警示具体原因。
    2. **连线补全**：JS 绘图增加了 300ms 的频率保护锁，彻底修复高频请求导致的线段丢失。
    3. **耗时必现**：采用了强制回写机制。如果仍为 0，请检查日志是否提示“配额超限”。
    4. **结构完整**：159单绝对闭环、空间调度算法、录单表单全部满血保留。
    """)

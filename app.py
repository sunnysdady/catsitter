...
def update_feishu_robust(record_id, sitter_name):
    """彻底修复 404：路径参数深度清洗"""
    token = get_feishu_token()
    if not token: return False, "Token获取失败"
    
    clean_app_token = APP_TOKEN.strip()
    clean_table_id = TABLE_ID.strip()
    clean_record_id = str(record_id).strip()
    
    # 重新构建 100% 准确的 RESTful URL
    url = f"https://open.feishu.cn/open-apis/bitable/v1/apps/{clean_app_token}/tables/{clean_table_id}/records/{clean_record_id}"
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            return False, f"业务报错: {res_json.get('msg')}"
        return False, f"路径错误(404)或无权限({r.status_code}): {r.text[:30]}"
...
def set_ui():
    st.markdown("""
        ...
        .diag-box { background: #fff1f0; border: 1px solid #ffa39e; padding: 10px; border-radius: 5px; font-family: monospace; font-size: 13px; margin-bottom: 5px; }
        </style>
        """, unsafe_allow_html=True)

# --- 5. 流程中心 ---

st.set_page_config(page_title="指挥中心 V22.0", layout="wide")
set_ui()
...
if st.session_state['page'] == "数据中心":
    ...
    with col_in1:
        with st.expander("批量导入 Excel"):
            up_file = st.file_uploader("Excel文件", type=["xlsx"])
            if up_file and st.button("🚀 启动批量录入"):
                ...
                for i, (_, row) in enumerate(df_up.iterrows()):
                    ...
                    requests.post(f"https://open.feishu.cn/open-apis/bitable/v1/apps/{clean_app_token}/tables/{clean_table_id}/records", headers={"Authorization": f"Bearer {tok}"}, json={"fields": f})
                    p_bar.progress((i + 1) / len(df_up))
                st.success("批量同步成功！"); st.session_state.pop('feishu_cache', None); st.rerun()
    with col_in2:
        with st.expander("✍️ 单条信息手动录入"):
            with st.form("single_manual"):
                a = st.text_input("详细地址*"); n = st.text_input("名"); s = st.date_input("开始日期"); e = st.date_input("结束日期")
                if st.form_submit_button("保存至云端"):
                    ...
                    requests.post(f"https://open.feishu.cn/open-apis/bitable/v1/apps/{APP_TOKEN.strip()}/tables/{TABLE_ID.strip()}/records", headers={"Authorization": f"Bearer {get_feishu_token()}"}, json={"fields": f})
    ...
    if not df_kb.empty and isinstance(date_range, tuple) and len(date_range) == 2:
        ...
        if st.button("✨ 1. 拟定派单方案"):
            ...
            st.session_state['final_plan_v22'] = pd.concat(all_plans) if all_plans else None
            st.success("✅ 方案拟定完成！已锁定 ID 传递链路。")

        if st.session_state.get('final_plan_v22') is not None:
            res_f = st.session_state['final_plan_v22']
            
            # --- 修复：重新找回日期与喂猫师筛选器 ---
            c_f1, c_f2 = st.columns(2)
            v_day = c_f1.selectbox("📅 选择查看日期", sorted(res_f['作业日期'].unique()))
            # 动态获取当前日期的喂猫师名单
            v_sitters = ["全部"] + sorted(res_f[res_f['作业日期'] == v_day]['喂猫师'].unique().tolist())
            v_sit = c_f2.selectbox("👤 筛选具体喂猫师", v_sitters)
                ...
                if st.button("✅ 2. 确认并强力同步飞书"):
                    ...
                    for i, (_, row) in enumerate(res_f.iterrows()):
                        if row.get('_system_id') and row.get('喂猫师'):
                            ok, msg = update_feishu_robust(row['_system_id'], row['喂猫师'])
                            if ok: suc += 1

...
st.set_page_config(page_title="指挥中心 V61.0", layout="wide")
set_ui()
...
with st.sidebar:
    ...
    sitters = ["梦蕊", "依蕊"]
    active = [s for s in sitters if st.checkbox(f"{s} (出勤)", value=True, key=f"v61_{s}")]
...
# 频道 3: 订单信息 (新增计费天数逻辑)
    ...
    if not df_i.empty:
        ...
        # 展示计费天数一列
        st.dataframe(df_i[['宠物名字', '计费天数', '详细地址', '喂猫师', '投喂频率', '备注']], use_container_width=True)
...
# 频道 4: 智能看板
    ...
    if not st.session_state['feishu_cache'].empty and isinstance(d_sel, tuple) and len(d_sel) == 2:
        if st.button("✨ 1. 拟定调度方案"):
            ...
            st.success("✅ 方案拟定完成！计费天数已自动对账。")

        if st.session_state.get('fp') is not None:
            st.download_button("📥 2. 导出 Excel 排单 (含归属明细)", data=generate_excel_v61(st.session_state['fp']), file_name="Cat_Dispatch_V61.xlsx")
            c1, c2 = st.columns(2)
...
# 频道 5: 帮助文档
    ...
    st.title("📖 V61 财务对账说明")
    st.markdown("""
    1. **计费天数**：在【订单信息】中，系统会根据您选择的区间自动算出“去喂了几次”，这即是计费天数。
    2. **频率公式**：1代表每天去，2代表隔天去。计费天数 = 区间内满足该频率的日期总和。
    3. **归属明细**：导出的 Excel 第二个 Sheet 依然保留了宠物归属明细，方便您随时查看或手动固化关系。
    """)

...
def get_normalized_address_v81(addr):
    """地址指纹：精准锁定大楼，确保同楼不拆单"""
...
def execute_smart_dispatch_spatial_v81(df, active_sitters):
    """聚类优先调度"""
    ...
    df['building_fingerprint'] = df['详细地址'].apply(get_normalized_address_v81)
...
def copy_to_clipboard_v81(text):
    """一键复制简报组件"""
    html_code = f"""
...
st.set_page_config(page_title="指挥中心 V81.0", layout="wide")
...
with st.sidebar:
    ...
    st.divider()
    s_filter = st.multiselect("🔍 状态过滤", options=["进行中", "已结束", "待处理"], default=["进行中", "待处理"])
    active_sitters = ["梦蕊", "依蕊"]
    active = [s for s in active_sitters if st.checkbox(f"{s} (出勤)", value=True, key=f"v81_{s}")]
...
if st.session_state['page'] == "数据中心":
    ...
    if not df_raw.empty:
        st.subheader("⚙️ 归属与状态维护")
        edit_dc = st.data_editor(df_raw[['宠物名字', '详细地址', '喂猫师', '订单状态']], 
                                 column_config={
                                     "喂猫师": st.column_config.SelectboxColumn("指定人", options=active_sitters),
        ...
        if st.button("🚀 提交同步修改"):
            ...
            st.success("已更新云端字段。"); st.session_state.pop('feishu_cache', None); st.rerun()
    st.divider()
    c1, c2 = st.columns(2)
    with c1:
        with st.expander("批量导入 (Excel)"):
            ...
            if up and st.button("🚀 推送"):
                ...
                for i, (_, r) in enumerate(du.iterrows()):
                    f = {"详细地址": str(r['详细地址']).strip(), "宠物名字": str(r.get('宠物名字', '小猫')).strip(), "服务开始日期": int(datetime.combine(pd.to_datetime(r['服务开始日期']), datetime.min.time()).timestamp()*1000), "服务结束日期": int(datetime.combine(pd.to_datetime(r['服务结束日期']), datetime.min.time()).timestamp()*1000), "订单状态": "进行中"}
    ...
    with c2:
        with st.expander("单条录单 (✍️)"):
            with st.form("man_v81"):
                a = st.text_input("地址*"); n = st.text_input("猫"); sd = st.date_input("开始"); ed = st.date_input("结束")
    ...
    if not df_raw.empty and isinstance(d_sel, tuple) and len(d_sel) == 2:
        ...
        if st.button("✨ 1. 拟定方案并开启交通图层"):
            ap = []; dk = execute_smart_dispatch_spatial_v81(df_kb, active); days = pd.date_range(d_sel[0], d_sel[1]).tolist()
            for d in days:
        ...
        if st.session_state.get('fp') is not None:
            st.metric("📊 最终派单总量", f"{len(st.session_state['fp'])} 单")
            st.download_button("📥 2. 导出全量 Excel", data=generate_excel_v81(st.session_state['fp']), file_name="Cat_Dispatch_V81.xlsx")
            c_f1, c_f2 = st.columns(2)
            vd = c_f1.selectbox("📅 选择日期", sorted(st.session_state['fp']['作业日期'].unique()))
            vs = c_f2.selectbox("👤 看板筛选", ["全部"] + sorted(active))
            ...
            copy_to_clipboard_v81(brief.replace('\n', '\\n'))
            st.text_area("📄 简报预览：", brief, height=180)

            # --- V81 地图升级：显示底图交通站点 POI ---
            cur_v = v_data[v_data['喂猫师'] == vs] if vs != "全部" else v_data
            map_d = cur_v.dropna(subset=['lng', 'lat'])
            if not map_d.empty:
                st.pydeck_chart(pdk.Deck(
                    # 使用 ROAD 样式以显示详细地铁/公交 POI
                    map_style=pdk.map_styles.ROAD,
                    initial_view_state=pdk.ViewState(longitude=map_d['lng'].mean(), latitude=map_d['lat'].mean(), zoom=14),
                    layers=[pdk.Layer(
                        "ScatterplotLayer",
                        map_d,
    ...
    st.title("📖 指挥中心操作指南 (V81交通辅助版)")
    st.info("💡 提示：底图已更换为街道模式。放大地图即可看到地铁图标和公交站位置。")

...
def set_ui():
    st.markdown("""
        ...
        [data-testid="stSidebar"] div[role="radiogroup"] label {
            ...
            padding: 30px 10px !important; border-radius: 14px !important;
            cursor: pointer; transition: all 0.2s ease-in-out; width: 100% !important;
        }
        [data-testid="stSidebar"] div[role="radiogroup"] label div[data-testid="stMarkdownContainer"] p {
            font-size: 20px !important; color: #333333 !important; font-weight: bold !important; text-align: center !important; margin: 0 !important;
        }
        ...
        with st.expander("单条快速补单"):
            with st.form("manual", clear_on_submit=True):
                ...
                if st.form_submit_button("保存到云端"):
                    ...
                    elif res == "duplicate": st.error("查重：记录已存在。")
                    st.session_state['feishu_cache'] = fetch_feishu_data()
    st.divider()
    if st.button("🔄 刷新预览云端数据"):
        ...
        if not df_v.empty:
            for c in ['服务开始日期', '服务结束日期']:
                df_v[c] = pd.to_datetime(df_v[c], unit='ms').dt.strftime('%Y-%m-%d')
        ...
        if 'period_plan' in st.session_state:
            ...
            if not v_data.empty:
                ...
                if st.button("✅ 确认同步全周期方案至飞书"):
                    ...
                    else: st.warning(f"⚠️ 同步结束，其中 {fail_count} 条失败。")

...
def get_travel_estimate_v103(origin, destination, mode_key):
    """高德 Web 服务计算路程与时间"""
...
def get_normalized_address_v103(addr):
...
def optimize_route_v103(df_sitter, mode_key):
    """【V103 核心】确保路程耗时数据 100% 物理注入"""
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = {executor.submit(get_travel_estimate_v103, f"{optimized[i]['lng']},{optimized[i]['lat']}", f"{optimized[i+1]['lng']},{optimized[i+1]['lat']}", mode_key): i for i in range(len(optimized)-1)}
...
def copy_to_clipboard_v103(text):
    html_code = f"""
...
st.set_page_config(page_title="指挥中心 V103.0", layout="wide")
...
with st.sidebar:
    ...
    active = [s for s in active_sitters if st.checkbox(f"{s} (出勤)", value=True, key=f"v103_{s}")]
...
# 智能看板：V103 终极修复版
                    ...
                    if not d_v.empty:
                        ...
                        if not d_v.empty:
                            ...
                            for s in active:
                                ...
                                if not stks.empty:
                                    res = optimize_route_v103(stks, mode_map[nav_mode])
    ...
    if st.session_state.get('fp') is not None:
        ...
        vs = c_f2.selectbox("👤 蓝/橙路线筛选", ["全部"] + sorted(active))
        v_data = st.session_state['fp'][st.session_state['fp']['作业日期'] == vd]
        
        # --- 全显简报预览修复 ---
        full_brief = f"📢 {vd} 任务简报\n"
        for s in active:
        ...
        copy_to_clipboard_v103(full_brief.replace('\n', '\\n'))
        st.text_area("📄 每一段任务耗时预览 (100% 同步)：", full_brief, height=250)
        
        # --- V103 地图核心修复：JSON 序列化修正 + 独立颜色全连线 ---
        # 强制将 Timestamp 转换为字符串，防止 TypeError
        ...
        if map_json_data:
            ...
            <script type="text/javascript">
                ...
                // 【V103 递归绘图】确保蓝/橙两色独立且 100% 连续
                function drawPathV103(idx, data, mode, map, sName) {{
                    ...
                    router.search([sData[idx].lng, sData[idx].lat], [sData[idx+1].lng, sData[idx+1].lat], function(status) {{
                        drawPathV103(idx + 1, data, mode, map, sName);
                    }});
                }}

                ["梦蕊", "依蕊"].forEach(s => drawPathV103(0, markers_data, "{mode_map[nav_mode]}", map, s));

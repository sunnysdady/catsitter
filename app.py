...
def get_travel_estimate_v129(origin, destination, mode_key):
    """高德 Web 服务算路"""
    ...
    try:
        ...
        return 0, 0, f"网路请求异常: {str(e)}"

def calculate_billing_days_v129(row, start_range, end_range):
    """【159单绝对财务逻辑】"""
    try:
...
def optimize_route_v129(df_sitter, mode_key, sitter_name, date_str):
    """【路径优化与测速物理锚定】"""
    ...
    for i in range(len(optimized) - 1):
        ...
        dist, dur, status = get_travel_estimate_v129(orig, dest, mode_key)
        if status != "SUCCESS": add_log(f"🚩 {sitter_name} 路段{i+1}: {status}", level="ERROR")
...
def execute_smart_dispatch_spatial_v129(df, active_sitters):
    """【复位 V99 空间分配算法】"""
...
def update_feishu_v129(record_id, field_name, value):
    try:
        r_auth = requests.post("https://open.feishu.cn/open-apis/auth/v3/tenant_access_token/internal", json={"app_id": APP_ID, "app_secret": APP_SECRET}, timeout=10)
        token = r_auth.json().get("tenant_access_token")
...
if st.session_state['feishu_cache'] is None:
    st.session_state['feishu_cache'] = fetch_feishu_v129()
...
with st.sidebar:
    ...
    st.divider()
    sitters_list = ["梦蕊", "依蕊"]
    active = [s for s in sitters_list if st.checkbox(f"{s} (执勤)", value=True, key=f"v129_{s}")]
...
if st.session_state['page'] == "数据中心":
    ...
    if not df_raw.empty:
        ...
        if isinstance(d_sel, tuple) and len(d_sel) == 2:
            df_raw['计费天数'] = df_raw.apply(lambda r: calculate_billing_days_v128(r, d_sel[0], d_sel[1]), axis=1) # 共享V128逻辑
    ...
    if not df_raw.empty:
        ...
        if st.button("🚀 提交同步并保存"):
            for i, row in edit_dc.iterrows():
                for f in ['订单状态', '喂猫师']:
                    if row[f] != df_raw.iloc[i][f]: update_feishu_v129(df_raw.iloc[i]['_system_id'], f, row[f])
...
# --- 7. 智能看板 ---
    ...
    st.title("🚀 数字化指挥大屏 (V129 纠偏修复版)")
    
    st.markdown('<div class="debug-tower">🗼 后台通讯塔 (全链路穿透普查)</div>', unsafe_allow_html=True)
    if st.session_state['system_logs']:
        for log in st.session_state['system_logs'][-15:]:
            st.write(f"`{log}`")
        if st.button("🧹 清空普查历史"): st.session_state['system_logs'] = []; st.rerun()
    else:
        st.info("📡 链路通畅。点击“开始拟定”触发对账。")
    ...
    if st.session_state['plan_state'] == "RUNNING":
        ...
        if not df_kb.empty:
            with st.status("🛸 穿透对账中...", expanded=True) as status:
                dk = execute_smart_dispatch_spatial_v129(df_kb, active)
                    ...
                    if not d_v.empty:
                        ...
                        if not d_v.empty:
                            with ThreadPoolExecutor(max_workers=5) as ex:
                                results = list(ex.map(get_coords_v129, d_v['详细地址']))
                            ...
                            for s in active:
                                ...
                                if not stks.empty:
                                    res = optimize_route_v129(stks, mode_map[nav_mode], s, d_str)
    ...
    if st.session_state.get('fp') is not None:
        ...
        <div id="map_box" style="width:100%; height:600px; border:3.5px solid #000; border-radius:15px; background:#f0f0f0;">
            <div id="no_coord" style="padding:20px; display:none; color:#ff4d4f; font-weight:bold;">⚠️ 解析成功率为 0%，请检查通讯塔。</div>
        </div>
    ...
    st.title("📖 V129 运营指引")
    st.markdown("""
    1. **语法纠偏**：修正了 `import` 错误，确保应用能正常启动。
    2. **并发加固**：保留 V128 全套状态初始化序列，解决 KeyError。
    3. **地图归位**：复位了高稳定性 JS 加载逻辑。
    """)

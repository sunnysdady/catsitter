...
def update_feishu_record_safe(record_id, sitter_name):
    ...
    try:
        ...
        if "application/json" in r.headers.get("Content-Type", ""):
            res_json = r.json()
            return res_json.get("code") == 0, res_json.get("msg", "OK")
        return False, "非 JSON 响应"
...
st.set_page_config(page_title="指挥中心 V16.0", layout="wide")
set_ui()
...
if st.session_state['page'] == "数据中心":
        ...
        with st.expander("✍️ 单条信息手动录入"):
            with st.form("single_cat_form"):
                ...
                if st.form_submit_button("💾 保存至云端"):
                    ...
                    requests.post(f"https://open.feishu.cn/open-apis/bitable/v1/apps/{APP_TOKEN}/tables/{TABLE_ID}/records", headers={"Authorization": f"Bearer {get_feishu_token()}"}, json={"fields": payload})
    ...
    if not df_kb.empty and isinstance(date_range, tuple) and len(date_range) == 2:
        ...
        if st.button("✨ 拟定派单方案"):
            ...
            st.session_state['final_plan_v16'] = pd.concat(all_plans) if all_plans else None
            st.success("✅ 拟定完成！")

        if st.session_state.get('final_plan_v16') is not None:
            res_f = st.session_state['final_plan_v16']

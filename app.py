...
def get_normalized_address_v90(addr):
...
def execute_smart_dispatch_spatial_v90(df, active_sitters):
    """空间聚类捆绑分配逻辑"""
    ...
    df['building_fingerprint'] = df['详细地址'].apply(get_normalized_address_v90)
...
def copy_to_clipboard_v90(text):
    html_code = f"""
...
st.set_page_config(page_title="指挥中心 V90.0", layout="wide")
...
with st.sidebar:
    ...
    active = [s for s in active_sitters if st.checkbox(f"{s} (出勤)", value=True, key=f"v90_{s}")]
...
# 智能看板：原生高德 API 渲染
    ...
    if not df_raw.empty and isinstance(d_sel, tuple) and len(d_sel) == 2:
        ...
        if st.button("✨ 1. 拟定方案并启动高德内核"):
            ap = []; dk = execute_smart_dispatch_spatial_v90(df_kb, active); days = pd.date_range(d_sel[0], d_sel[1]).tolist()
            for d in days:
            ...
            st.success("✅ 拟定完成！100% 高德原生环境已激活。")

        if st.session_state.get('fp') is not None:
            st.metric("📊 最终派单总量 (财务闭环)", f"{len(st.session_state['fp'])} 单")
            st.download_button("📥 2. 导出全量 Excel", data=generate_excel_v90(st.session_state['fp']), file_name="Cat_Dispatch_V90.xlsx")
            c_f1, c_f2 = st.columns(2)
            vd = c_f1.selectbox("📅 选择日期", sorted(st.session_state['fp']['作业日期'].unique()))
            vs = c_f2.selectbox("👤 地图人员筛选", ["全部"] + sorted(active))
            ...
            copy_to_clipboard_v90(brief.replace('\n', '\\n'))
            st.text_area("📄 简报预览：", brief, height=180)

            cur_v = v_data[v_data['喂猫师'] == vs] if vs != "全部" else v_data
            map_d = cur_v.dropna(subset=['lng', 'lat']).to_dict('records')
            
            # --- V90 核心突破：直接调用高德 JS API ---
            if map_d:
                markers_json = json.dumps(map_d)
                amap_html = f"""
                ...
                <script type="text/javascript">
                    const map = new AMap.Map('container', {{ zoom: 15, center: [{map_d[0]['lng']}, {map_d[0]['lat']}] }});
                    ...
                    markers.forEach(m => {{
                        const marker = new AMap.Marker({{
                            ...
                            title: m.宠物名字,
                            map: map,
                            content: `<div style="width:12px; height:12px; background:${{m.color}}; border:2px solid white; border-radius:50%; box-shadow:0 0 5px rgba(0,0,0,0.3);"></div>`
                        }});

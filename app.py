...
def set_ui():
    st.markdown("""
        ...
        /* 设置区域的文字间距优化 */
        [data-testid="stSidebar"] .stMarkdown p { font-weight: bold !important; font-size: 14px !important; }
...
with st.sidebar:
    ...
    # 1. 核心功能大按钮
    if st.button("📂 数据录入中心"): st.session_state['page'] = "数据中心"
    if st.button("🚀 智能排单看板"): st.session_state['page'] = "智能看板"
    
    # 2. 调度设置上移 (仅在看板页显示，确保首屏可见)
    if st.session_state['page'] == "智能看板":
        st.divider()
        st.subheader("⚙️ 调度快速配置")
        active_sitters = ["梦蕊", "依蕊"]
        current_active = [s for s in active_sitters if st.checkbox(f"{s} (出勤)", value=True)]
        date_range = st.date_input("📅 派单日期范围", value=(datetime.now(), datetime.now() + timedelta(days=2)))
    else:
        current_active = ["梦蕊", "依蕊"]
        date_range = (datetime.now(), datetime.now() + timedelta(days=2))

    # 3. 动态 Spacer 保持帮助按钮置底
    # 如果在看板页，高度设低一点；在数据页，高度设高一点
    spacer_height = "15vh" if st.session_state['page'] == "智能看板" else "45vh"
    st.markdown(f'<div style="height: {spacer_height};"></div>', unsafe_allow_html=True)
    
    st.divider()
    if st.button("📖 使用帮助 & 日志"): st.session_state['page'] = "帮助"
...
if st.session_state['page'] == "数据中心":
    ...
    st.title("📖 使用指南与作业标准")
    t1, t2 = st.tabs(["💡 操作 SOP", "📜 版本日志"])
    with t1:
        st.info("**核心标准**：飞书负责存底，看板负责排期。团队需严格按照‘微信简报’顺序执行作业。")
        st.markdown("""
        - **1. 录入**：确保详细地址准确，辅助定位。
        - **2. 排单**：上午拟定方案，核实地图点位。
        - **3. 同步**：点击回写后，飞书会更新责任人字段。
        """)
    with t2:
        st.subheader("📅 2026-02-11 (V2.3)")
        st.markdown("""
        * **按钮巨幕化**：侧边栏按钮高度提升至 80px，字体放大至 20px 加粗，适配高频点击需求。
        * **布局首屏化**：将“调度设置”上移，确保进入看板页无需下划即可配置伙伴与日期。
        * **弹性 Spacer**：优化了底部帮助按钮的定位逻辑，解决因设置项增多导致的重叠问题。
        """)

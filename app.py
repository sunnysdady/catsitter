...
# ==========================================
# --- 【V152 入口保险锁：V144 逻辑全量回归】 ---
# ==========================================
def init_session_state_v152():
    """彻底终结数据失踪，确保全链路可视化 100% 闭环"""
...
# --- 3. 核心底座逻辑 (回滚至 V144 高精算法) ---

def haversine_v152(lon1, lat1, lon2, lat2, mode):
    """【精度自愈】球面直线算法"""
    R = 6371000 
...
def get_travel_estimate_v152(origin, destination, mode_key):
    """【回归版】算路大脑"""
...
def get_normalized_address_v152(addr):
    """【回归版】V99 地址指纹"""
...
def calculate_billing_v152(row, start_range, end_range):
    """【回归版】159单财务计费"""
    try:
...
def optimize_route_v152(df_sitter, mode_key, sitter_name, date_str, start_addr):
    """【回归版】派单测速引擎，物理锁定公里数"""
    ...
    start_pt, _ = get_coords_v152(start_addr)
    ...
    t_d, t_t = 0, 0
    if start_pt:
        d0, t0, s0 = get_travel_estimate_v152(f"{start_pt[0]},{start_pt[1]}", f"{optimized[0]['lng']},{optimized[0]['lat']}", mode_key)
        if s0 != "SUCCESS": d0, t0 = haversine_v152(start_pt[0], start_pt[1], optimized[0]['lng'], optimized[0]['lat'], mode_key)
        optimized[0]['prev_dur'] = t0; t_d += d0; t_t += t0

    for i in range(len(optimized) - 1):
        d, t, s = get_travel_estimate_v152(f"{optimized[i]['lng']},{optimized[i]['lat']}", f"{optimized[i+1]['lng']},{optimized[i+1]['lat']}", mode_key)
        if s != "SUCCESS": d, t = haversine_v152(optimized[i]['lng'], optimized[i]['lat'], optimized[i+1]['lng'], optimized[i+1]['lat'], mode_key)
        optimized[i]['next_dist'], optimized[i]['next_dur'] = d, t
        t_d += d; t_t += t

    st.session_state['commute_stats'][f"{date_str}_{sitter_name}"] = {"dist": t_d, "dur": t_t}
    add_log(f"✅ {sitter_name} ({date_str}) 测算完毕: {t_d/1000:.2f}km", level="INFO")
...
def set_ui_v152():
    st.markdown("""
        ...
        .sidebar-v144-header { font-size: 0.85rem; font-weight: 800; color: #777; margin: 1.2rem 0 0.5rem 0; letter-spacing: 1.2px; }
        [data-testid="stSidebar"] p, [data-testid="stSidebar"] span, [data-testid="stSidebar"] label { color: #ffffff !important; }
        
        /* 灰色圆角矩阵功能块 */
        .v144-box [data-testid="stVerticalBlock"] div.stButton > button { 
            ...
            background-color: #2d2d2d !important; color: #ffffff !important; margin-bottom: 12px !important; transition: 0.25s all;
        }
        .v144-box div.stButton > button:hover { background-color: #444 !important; border-color: #007bff !important; }
        ...
        .v144-card h4 { color: #888 !important; font-size: 14px !important; margin: 0 0 6px 0 !important; }
        .v144-card p { font-size: 28px !important; font-weight: 900 !important; color: #111 !important; margin: 0 !important; }
        
        /* 日志黑匣子 */
        .terminal-v144 { background-color: #111; color: #00ff00; padding: 12px; border-radius: 10px; font-family: monospace; font-size: 11px; height: 260px; overflow-y: auto; border: 1px solid #333; }
        </style>
...
with st.sidebar:
    # 视角锁定 (置顶)
    st.markdown('<div class="sidebar-v144-header">👤 视角权限锁定</div>', unsafe_allow_html=True)
    st.session_state['viewport'] = st.selectbox("角色", ["管理员模式", "梦蕊模式", "依蕊模式"], label_visibility="collapsed")
    st.divider()

    # 导航中心 (圆角灰色按钮)
    st.markdown('<div class="sidebar-v144-header">🧭 功能导航中心</div>', unsafe_allow_html=True)
    st.markdown('<div class="v144-box">', unsafe_allow_html=True)
    if st.button("📊 派单对账大屏"): st.session_state['page'] = "派单看板"
    if st.button("📂 资料同步管理"): st.session_state['page'] = "数据中心"
    if st.button("📖 平台使用手册"): st.session_state['page'] = "帮助"
    st.markdown('</div>', unsafe_allow_html=True)
    st.divider()

    # 核心配置
    st.markdown('<div class="sidebar-v144-header">⚙️ 派单服务参数</div>', unsafe_allow_html=True)
    ...
    # 日志通讯塔 (底层折叠)
    st.divider()
    st.markdown('<div class="sidebar-v144-header">📡 系统自检黑匣子</div>', unsafe_allow_html=True)
    with st.expander("展开影子日志", expanded=False):
        logs_txt = "\n".join(st.session_state['system_logs'][-40:])
        st.markdown(f'<div class="terminal-v144">{logs_txt}</div>', unsafe_allow_html=True)
...
def fetch_data_v152():
    try:
        ...
        r = requests.get(url, headers={"Authorization": f"Bearer {tk}"}, params={"page_size": 500}, timeout=15).json()
...
if st.session_state['feishu_cache'] is None: st.session_state['feishu_cache'] = fetch_data_v152()
...
if st.session_state['page'] == "数据中心":
    ...
    if not df.empty:
        ...
        if isinstance(st.session_state['r'], tuple) and len(st.session_state['r']) >= 2:
            df['计费单量'] = df.apply(lambda r: calculate_billing_v152(r, st.session_state['r'][0], st.session_state['r'][1]), axis=1)
            st.metric("分析周期内累计单量", f"{df['计费单量'].sum()} 次")
...
# --- 7. 派单看板：V144 逻辑满血回归版 ---
    ...
    pool_c = len(df_raw); matched_c = 0; coord_c = 0
    if st.session_state.get('fp') is not None:
        matched_c = len(st.session_state['fp']); coord_c = len(st.session_state['fp'].dropna(subset=['lng']))
    ...
    st.markdown(f"""<div style="background:#f8f9fa; padding:15px; border-radius:12px; border:1px solid #ddd; display:flex; justify-content:space-around; margin-bottom:20px;">
        <div style="text-align:center;"><p style="font-size:0.8rem; color:#666;">飞书总池</p><p style="font-size:1.1rem; font-weight:800; color:#000;">{pool_c}</p></div>
        <div style="text-align:center;"><p style="font-size:0.8rem; color:#666;">周期内订单</p><p style="font-size:1.1rem; font-weight:800; color:#007bff;">{matched_c}</p></div>
        <div style="text-align:center;"><p style="font-size:0.8rem; color:#666;">坐标有效点</p><p style="font-size:1.1rem; font-weight:800; color:#28a745;">{coord_c}</p></div>
    ...
    if st.session_state['plan_state'] == "RUNNING":
        # IndexError 安全锁 (V144 核心)
        if not isinstance(st.session_state['r'], tuple) or len(st.session_state['r']) < 2:
            st.warning("⚠️ 请在日历中点选完整的【起始】和【结束】日期。")
            st.session_state['plan_state'] = "IDLE"; st.stop()

        if not df_raw.empty:
            prog = st.progress(0.0, text="同步高德路网轴中...")
            with st.status("正在回归执行高精测速...", expanded=True) as status:
                # 复位 V99 空间调度 (同楼不拆单)
                sitters = ["梦蕊", "依蕊"]
                df_raw['building_fp'] = df_raw['详细地址'].apply(get_normalized_address_v152)
                s_load = {s: 0 for s in sitters}
                unassigned = ~df_raw['喂猫师'].isin(sitters)
                if unassigned.any():
                ...
                for idx, d in enumerate(days):
                    ...
                    prog.progress((idx+1)/len(days), text=f"正在锁定: {d.strftime('%Y-%m-%d')}")
                    ...
                    if not d_v.empty:
                        ...
                        if not d_v.empty:
                            with ThreadPoolExecutor(max_workers=5) as ex:
                                results = list(ex.map(get_coords_v152, d_v['详细地址']))
                            ...
                            for s in sitters:
                                ...
                                if not stks.empty:
                                    res = optimize_route_v152(stks, st.session_state['travel_mode'], s, d.strftime('%Y-%m-%d'), st.session_state['departure_point'])
                                    res['作业日期'] = d.strftime('%Y-%m-%d'); all_plans.append(res)
                st.session_state['fp'] = pd.concat(all_plans) if all_plans else None
                status.update(label="✅ 复原计算完成！", state="complete")
    ...
    if st.session_state.get('fp') is not None:
        ...
        for i, sn in enumerate(show_names):
            ...
            with [c1, c2][i%2]:
                st.markdown(f"""<div class="v144-card"><h4>{sn} 派单统计</h4><p>服务：{len(day_all[day_all['喂猫师']==sn])} 单</p><p style="color:#007bff;">时长：{int(stt['dur'])} 分钟</p><p>路程：{stt['dist']/1000:.2f} km</p></div>""", unsafe_allow_html=True)
        ...
        for _, r in v_data.iterrows():
            ...
            if seq == 1 and pd_dur > 0: line += f" (起点首段 {int(pd_dur)}分)"
        ...
        if not map_clean.empty:
            ...
            st.error("⚠️ 选定视角坐标解析失败，地图无法渲染，请检查黑匣子日志。")

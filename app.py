...
# 【V122 核心】双 Key 驱动，解决 USERKEY_PLAT_NOMATCH
AMAP_KEY_WS = st.secrets.get("AMAP_KEY_WS", st.secrets.get("AMAP_KEY", "")).strip() 
AMAP_KEY_JS = st.secrets.get("AMAP_KEY_JS", st.secrets.get("AMAP_KEY", "")).strip() 
AMAP_JS_CODE = st.secrets.get("AMAP_JS_CODE", "").strip()

if 'system_logs' not in st.session_state: st.session_state['system_logs'] = []

# --- 2. 核心调度与测速引擎 (复位 V99 空间算法) ---

def get_travel_estimate_v122(origin, destination, mode_key):
    """【V122 诊断增强】使用 Web 服务 Key 计算耗时"""
    ...
    try:
        ...
        if r['status'] == '1':
            ...
            if info == "USERKEY_PLAT_NOMATCH":
                return 0, 0, "错误：Key 类型不匹配。请确认 AMAP_KEY_WS 是【Web 服务】类型。"
            return 0, 0, f"高德报错: {info}"
    except Exception as e:
        return 0, 0, f"算路请求异常: {str(e)}"

def get_normalized_address_v122(addr):
    """【复位 V99】地址识别逻辑"""
...
def optimize_route_v122(df_sitter, mode_key):
    """【V122 修复】物理回写测速结果"""
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for i in range(len(optimized) - 1):
            ...
            futures[executor.submit(get_travel_estimate_v122, orig, dest, mode_key)] = i
...
def execute_smart_dispatch_spatial_v122(df, active_sitters):
    """【复位 V99】空间聚类算法"""
    ...
    df['building_fingerprint'] = df['详细地址'].apply(get_normalized_address_v122)
...
st.set_page_config(page_title="指挥中心 V122.0", layout="wide")
...
with st.sidebar:
    ...
    d_sel = st.date_input("日期范围", value=st.session_state.get('r', (td, td + timedelta(days=1))))
    st.divider()
    active_sitters = ["梦蕊", "依蕊"]
    active = [s for s in active_sitters if st.checkbox(f"{s} (出勤)", value=True, key=f"v122_{s}")]
...
# --- 7. 智能看板 (双核修复版) ---
    ...
    st.title("🚀 数字化指挥大屏 (双核动力版)")
    
    # 【新增】诊断日志
    if st.session_state['system_logs']:
        with st.expander("⚠️ 运行诊断日志", expanded=True):
    ...
    if st.session_state['plan_state'] == "RUNNING":
        ...
        if not df_kb.empty:
            with st.status("🛸 空间绑定引擎正在测速...", expanded=True) as status:
                dk = execute_smart_dispatch_spatial_v122(df_kb, active)
                    ...
                    if not d_v.empty:
                        ...
                        if not d_v.empty:
                            ...
                            for s in active:
                                ...
                                if not stks.empty:
                                    res = optimize_route_v122(stks, mode_map[nav_mode])
    ...
    if st.session_state.get('fp') is not None:
        ...
        for i, s in enumerate(show_sitters):
            ...
            if not s_sum.empty:
                ...
                card_html = f"""<div class="commute-card"><h4>👤 {s} 态势</h4><p>当日单量：{t_count} 单</p><p style="color: #00ff00 !important;">预计耗时：{int(t_dur)} 分钟</p><p style="color: #00d4ff !important;">总行程：{t_dist:.1f} km</p></div>"""
                [c_m1, c_m2][i % 2].markdown(card_html, unsafe_allow_html=True)
        
        st.text_area("📄 简报预览 (数据已锚定)：", f"📢 {vd} 指战简报 ({vs})\n" + "\n".join([f"{r['拟定顺序']}. {r['宠物名字']}-{r['详细地址']} ➡️ ({r['next_dur']}分钟)" for _,r in v_data.iterrows()]), height=200)
                ...
                (function() {{
                    ...
                    function drawChain(idx, sData, mode, map) {{
                        ...
                        router.search([sData[idx].lng, sData[idx].lat], [sData[idx+1].lng, sData[idx+1].lat], function() {{
                            setTimeout(() => drawChain(idx + 1, sData, mode, map), 300);
                        }});
                    }}
    ...
    st.title("📖 V122 洛阳指战员手册")
    st.markdown("""
    1. **双核驱动**：看板现支持独立配置 `AMAP_KEY_WS` (Web服务/算路) 与 `AMAP_KEY_JS` (Web端JS/绘图)。
    2. **空间调度**：找回了 V99 空间算法，同楼不拆单。
    3. **耗时真实**：物理索引回灌确保汇总卡片数据 100% 来源真实测算。
    """)
